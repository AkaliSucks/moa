...
from moa.services.retained_source_kakeraloot_settings_reprojection_executor import (
    RetainedSourceKakeralootSettingsReprojectionError,
...
def database_path(tmp_path):
    path = tmp_path / "kakeraloot-settings-reprojection.sqlite3"
...
def _fixture(connection: sqlite3.Connection, historical_generations: int = 2):
    ...
    import_id = int(
        connection.execute(
            ...
            "VALUES ('kakeraloot_settings', 'discord', ?, 'retained settings')",
            (NOW,),
        ).lastrowid
    )
    observation_id = int(
        connection.execute(
            "INSERT INTO kakeraloot_settings_observations (server_context_id, loot_cost, "
            "quantity_quality_base_cost, quantity_quality_level_increment, observed_at, "
            "import_event_id) VALUES (1, 0, 0, 0, ?, ?)",
            (NOW, import_id),
        ).lastrowid
    )
    connection.execute(
    ...
    for generation_id in range(1, historical_generations + 1):
        ...
        connection.execute(
            ...
            "(?, ?, ?, ?, 'kakeraloot_settings_observations', ?, 'completed', ?, ?, ?, ?)",
            (
                source_id,
                generation_id,
    ...
    assert dict(admission.payloads[0].fields)["loot_cost"] == 0
    return admission
...
def _execute(connection, admission):
    return RetainedSourceKakeralootSettingsReprojectionExecutor().execute(
        connection, admission, COMPLETED_AT
    )


def test_creates_only_the_missing_current_link_preserves_zero_and_replay_is_no_write(
    database_path,
) -> None:
    with connect(database_path) as connection:
        ...
        assert result.source_family == "kakeraloot_settings"
...
def test_requires_caller_owned_transaction(database_path) -> None:
    with connect(database_path) as connection:
        ...
        connection.commit()
        before = _dump(connection)
        with pytest.raises(RetainedSourceKakeralootSettingsReprojectionError, match="caller-owned"):
...
@pytest.mark.parametrize(
    "forge",
    (
        lambda admission: replace(admission, source_family="server_settings"),
        ...
        lambda admission: replace(
            ...
            payloads=(
                replace(
                    ...
                    fields=tuple(
                        (name, False if name == "loot_cost" else value)
                        for name, value in admission.payloads[0].fields
                    ),
                ),
            ),
        ),
...
def test_rejects_stale_forged_or_malformed_admission_without_writing(database_path, forge) -> None:
    with connect(database_path) as connection:
        connection.execute("BEGIN IMMEDIATE")
        admission = _fixture(connection)
        before = _dump(connection)
        with pytest.raises(RetainedSourceKakeralootSettingsReprojectionError):
...
@pytest.mark.parametrize(
    ...
    (
        ...
        "UPDATE kakeraloot_settings_observations SET loot_cost = 'UNKNOWN'",
        "UPDATE kakeraloot_settings_observations SET import_event_id = 999",
        "UPDATE kakeraloot_settings_observations SET server_context_id = 999",
    ),
...
def test_revalidates_every_historical_generation_on_replay(database_path) -> None:
    with connect(database_path) as connection:
        ...
        before = _dump(connection)
        with pytest.raises(
            RetainedSourceKakeralootSettingsReprojectionError, match="historical evidence"
        ):
            _execute(connection, admission)
        assert _dump(connection) == before
...
@pytest.mark.parametrize(
    ...
    (
        ("claimed", "catalog.kakeraloot_settings", "expected", None, None),
        (
            "completed",
            "catalog.kakeraloot_settings",
            "wrong-slot",
        ...
        ("completed", "catalog.kakeraloot_settings", "expected", "wishlist_observations", 1),
        (
            "completed",
            "catalog.kakeraloot_settings",
            "expected",
