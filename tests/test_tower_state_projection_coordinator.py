...
def _insert_historical_completed_link(
    ...
    connection.execute(
        ...
        ) VALUES (?, 1, 'catalog.tower_state', ?,
                  'tower_state_observations', 987, 'completed', ?, ?, ?, ?)
        """,
...
def test_generation_one_processing_persists_atomic_tower_projection(tmp_path) -> None:
...
def test_generation_two_ignores_history_and_replay_is_durable_no_write(tmp_path) -> None:
    ...
    with connect(database_path) as connection:
        ...
        ] == [(2, "tower_state_observations", first.tower_state_observation_id, "completed")]
...
def test_generation_two_replay_fails_when_only_historical_link_exists(tmp_path) -> None:
    ...
    with pytest.raises(
        TowerStateProjectionIntegrityError,
        match="inconsistent Tower projection link",
    ):
...
def test_current_generation_replay_rejects_target_import_ownership_corruption(
    ...
    if mutation == "wrong_import":
        second = catalog.import_tower_state(
            TOWER_STATE, "Server", "Account", "second", "test"
        )
        with connect(database_path) as connection:
            second_id = connection.execute(
                "SELECT id FROM tower_state_observations WHERE import_event_id = ?",
    ...
    with pytest.raises(TowerStateProjectionTargetError):
        _coordinate(coordinator, source_event_id, None)
...
def test_coordinator_uses_supplied_helper_without_public_wrapper_nesting(
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("coordinator called public Tower State wrapper")
        ),
    )
