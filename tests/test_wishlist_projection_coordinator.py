...
def _insert_historical_completed_link(
    ...
    connection.execute(
        ...
        ) VALUES (?, 1, 'catalog.wishlist', ?,
                  'wishlist_observations', 987, 'completed', ?, ?, ?, ?)
        """,
...
def test_historical_wishlist_link_is_isolated_from_current_generation_and_replay(
    tmp_path,
) -> None:
    database_path, _catalog, discord, coordinator = _repositories(tmp_path)
    source_event_id, attempt_id = _receive_and_begin(discord)
    ...
    with connect(database_path) as connection:
        ...
        assert [tuple(row) for row in current_link] == [
            (2, "wishlist_observations", first.wishlist_observation_id, "completed")
        ]
...
def test_current_generation_target_validation_failure_preserves_history_and_processing(
    ...
    monkeypatch.setattr(
        coordinator,
        "_validate_wishlist_target",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            WishlistProjectionTargetError("forced target-validation failure")
        ),
    )

    with pytest.raises(WishlistProjectionTargetError, match="target-validation"):
