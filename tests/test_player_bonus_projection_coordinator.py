...
from moa.services.player_bonus_projection_coordinator import (
    PlayerBonusProjectionCoordinator,
    PlayerBonusProjectionDatabasePathError,
    PlayerBonusProjectionIntegrityError,
...
PLAYER_BONUS = PlayerBonusSnapshot(
    metrics=(
        ...
        PlayerBonusMetric(label="Spawn bonus for wishes", detail="+210% ($k + $bw + slash)"),
    ),
...
def _repositories(tmp_path):
    database_path = tmp_path / "player-bonus-coordinator.db"
    catalog = CatalogRepository(database_path)
    discord = DiscordMessageRepository(database_path)
    return database_path, catalog, discord, PlayerBonusProjectionCoordinator(catalog, discord)
...
def _receive_and_begin(discord, *, suffix="one"):
    ...
    received = discord.receive_message(
        ...
        raw_text="player bonus payload",
        payload_json='{"content":"player bonus payload"}',
...
def _coordinate(
    ...
    *,
    ...
    return coordinator.coordinate_player_bonus(
...
def _assert_bonus_values(connection, result, state):
    observation = connection.execute(
        ...
        """,
        (result.player_bonus_observation_id,),
    ).fetchone()
    assert observation["id"] == result.player_bonus_observation_id
...
@pytest.mark.parametrize("state", [PLAYER_BONUS, BOUNDARY_PLAYER_BONUS])
def test_first_processing_writes_one_player_bonus_projection_and_preserves_values(
    tmp_path, state
) -> None:
    ...
    assert result == PlayerBonusProjectionResult(
        imported_count=1,
        import_event_id=result.import_event_id,
        player_bonus_observation_id=result.player_bonus_observation_id,
        replay_skipped=False,
        durable_success_recorded=True,
        projection_target=("player_bonus_observations", result.player_bonus_observation_id),
    )
    with connect(database_path) as connection:
        ...
        assert tuple(event) == (
            ...
            "player_bonus",
            "discord",
            "player bonus payload",
            OBSERVED_AT.isoformat(),
        )
        _assert_bonus_values(connection, result, state)
        assert tuple(
            connection.execute("SELECT name, normalized_name FROM server_contexts").fetchone()
        ) == ("Server", "server")
        assert tuple(
            connection.execute("SELECT name, normalized_name FROM account_contexts").fetchone()
        ) == ("Account", "account")
...
def test_attribution_failures_leave_processing_state_and_rows_unchanged(
    ...
    with pytest.raises(PlayerBonusProjectionIntegrityError, match=message):
    ...
    assert before["counts"]["player_bonus_observations"] == 0
...
def test_failure_after_catalog_writes_rolls_back_and_retry_succeeds_once(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: (_ for _ in ()).throw(RuntimeError("forced bonus failure")),
    )

    with pytest.raises(RuntimeError, match="forced bonus failure"):
        _coordinate(coordinator, source_event_id, attempt_id)
    with connect(database_path) as connection:
        assert _counts(connection)["import_events"] == 0
        assert _counts(connection)["player_bonus_observations"] == 0
    ...
    with connect(database_path) as connection:
        ...
        assert _counts(connection)["player_bonus_observations"] == 1
...
def test_pre_existing_contexts_are_reused_and_preserved(tmp_path) -> None:
    ...
    catalog.import_player_bonus(PLAYER_BONUS, "Server", "Account", "existing", "discord")
    with connect(database_path) as connection:
    ...
    with connect(database_path) as connection:
        ...
        assert _counts(connection)["player_bonus_observations"] == 2
...
def test_succeeded_replay_after_reconstructing_coordinator_returns_existing_ids_and_inserts_nothing(
    ...
    reconstructed = PlayerBonusProjectionCoordinator(
        reconstructed_catalog, reconstructed_discord
    )
    replay = _coordinate(reconstructed, source_event_id, None)

    assert replay == PlayerBonusProjectionResult(
        imported_count=0,
        import_event_id=first.import_event_id,
        player_bonus_observation_id=first.player_bonus_observation_id,
        replay_skipped=True,
        durable_success_recorded=True,
...
def test_succeeded_replay_validates_completed_target_integrity(tmp_path, mutation: str) -> None:
    ...
    with connect(database_path) as connection:
        if mutation == "missing_target":
            connection.execute(
                "DELETE FROM player_bonus_observations WHERE id = ?",
                (first.player_bonus_observation_id,),
            )
            ...
            second = catalog.import_player_bonus(
                PLAYER_BONUS, "Server", "Account", "second", "discord"
            )
            second_id = connection.execute(
                "SELECT id FROM player_bonus_observations WHERE import_event_id = ?",
            ...
            second = catalog.import_player_bonus(
                PLAYER_BONUS, "Server", "Other Account", "other", "discord"
            )
            second_id = connection.execute(
                "SELECT id FROM player_bonus_observations WHERE import_event_id = ?",
                (second.import_event_id,),
            ...
            connection.execute(
                "UPDATE player_bonus_observations SET metrics_json = ? WHERE id = ?",
                ('[{"label":"wrong","detail":"wrong"}]', first.player_bonus_observation_id),
            )
        elif mutation == "field":
            connection.execute(
                "UPDATE player_bonus_observations SET rolls_per_hour_bonus = 999 WHERE id = ?",
                (first.player_bonus_observation_id,),
            )

    with pytest.raises((PlayerBonusProjectionIntegrityError, PlayerBonusProjectionTargetError)):
...
def test_claimed_or_conflicting_first_link_fails_closed(tmp_path, mutation: str = "claimed") -> None:
    ...
    with pytest.raises(PlayerBonusProjectionIntegrityError):
        _coordinate(coordinator, source_event_id, attempt_id)
...
def test_attempt_ownership_and_lifecycle_are_validated(tmp_path) -> None:
    ...
    with pytest.raises(PlayerBonusProjectionStateError, match="another source event"):
    ...
    with pytest.raises(PlayerBonusProjectionStateError, match="not processing"):
        _coordinate(coordinator, other_event_id, other_attempt_id)
    with pytest.raises(PlayerBonusProjectionStateError, match="active processing attempt"):
        _coordinate(coordinator, source_event_id, None)
...
def test_succeeded_source_event_rejects_supplied_attempt(tmp_path) -> None:
    ...
    with pytest.raises(PlayerBonusProjectionStateError, match="already succeeded"):
        _coordinate(coordinator, source_event_id, attempt_id)


def test_invalid_replay_does_not_create_replacement_rows(tmp_path) -> None:
    database_path, _catalog, discord, coordinator = _repositories(tmp_path)
    source_event_id, attempt_id = _receive_and_begin(discord)
    _record_attribution(discord, source_event_id)
    first = _coordinate(coordinator, source_event_id, attempt_id)
    ...
    with pytest.raises(PlayerBonusProjectionTargetError):
        _coordinate(coordinator, source_event_id, None)
...
def test_database_path_mismatch_is_rejected_before_writes(tmp_path) -> None:
    ...
    with pytest.raises(PlayerBonusProjectionDatabasePathError):
        PlayerBonusProjectionCoordinator(catalog, discord)
