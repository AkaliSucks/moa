"""Transactional coordination for durable Discord `$oq` projections."""
...
class SphereResultProjectionCoordinatorError(RuntimeError):
    """Base error for a sphere-result projection coordination failure."""


class SphereResultProjectionStateError(SphereResultProjectionCoordinatorError):
    """Raised when a source event or processing attempt is not usable."""


class SphereResultProjectionIntegrityError(SphereResultProjectionCoordinatorError):
    """Raised when durable sphere-result projection state cannot be trusted."""


class SphereResultProjectionTargetError(SphereResultProjectionIntegrityError):
    """Raised when a sphere-result projection target is missing or mismatched."""


class SphereResultProjectionDatabasePathError(SphereResultProjectionCoordinatorError, ValueError):
    """Raised when the repositories do not point at one database."""


@dataclass(frozen=True, slots=True)
class SphereResultProjectionResult:
    """The durable outcome of one coordinated `$oq` sphere-result projection."""
...
class SphereResultProjectionCoordinator:
    ...
    def coordinate_sphere_result(
        ...
        *,
        ...
        projection_slot = self._sphere_result_slot(server, account)
        ...
        try:
            ...
            if set(links) - {expected_key}:
                raise SphereResultProjectionIntegrityError(
            ...
            if existing is not None:
                if str(existing["state"]) == "claimed":
                    raise SphereResultProjectionIntegrityError(
                        f"sphere-result projection for source event {source_event_id} is still claimed"
                    )
                raise SphereResultProjectionIntegrityError(
                    f"source event {source_event_id} already has a sphere-result projection"
                )
            ...
            imported = self._catalog._import_sphere_result_with_connection(
                connection,
                state=state,
                server=server,
                account=account,
            ...
            observation_id = self._positive_id(
                ...
                "sphere_result_observation_id",
            )
            target = (self._PROJECTION_TABLE, observation_id)
            self._validate_sphere_result_target(
                connection,
            ...
            if success.attempt_status != "succeeded" or success.source_event_status != "succeeded":
                raise SphereResultProjectionStateError(
                    f"processing success was not recorded for source event {source_event_id}"
                )
            connection.commit()
            return SphereResultProjectionResult(
                imported_count=1,
                import_event_id=import_event_id,
                sphere_result_observation_id=observation_id,
    ...
    def _coordinate_replay(
        ...
        *,
        ...
        if import_event is None or str(import_event["kind"]) != self._IMPORT_KIND:
            raise SphereResultProjectionTargetError(
                f"legacy sphere-result import event {import_event_id} for source event {event['id']} is missing or wrong"
            )
        ...
        if set(links) != {key}:
            raise SphereResultProjectionIntegrityError(
                f"succeeded source event {event['id']} has an inconsistent sphere-result projection link"
            )
        link = links[key]
        if str(link["state"]) != "completed":
            raise SphereResultProjectionIntegrityError(
                f"sphere-result projection for source event {event['id']} is not completed"
            )
        ...
        if projection_row_id is None:
            raise SphereResultProjectionTargetError(
                f"sphere-result projection for source event {event['id']} has no target"
            )
    ...
    def _complete_projection_link(
        ...
        *,
        ...
        if table not in self._TARGET_TABLES or isinstance(row_id, bool) or row_id <= 0:
            raise SphereResultProjectionIntegrityError(
                "sphere-result import returned an invalid projection target"
            )
        ...
        if updated.rowcount != 1:
            raise SphereResultProjectionIntegrityError(
                "sphere-result projection link could not be completed"
            )

    def _validate_sphere_result_target(
        self,
        connection: sqlite3.Connection,
        *,
        observation_id: int,
        ...
        if self._PROJECTION_TABLE not in self._TARGET_TABLES:
            raise SphereResultProjectionIntegrityError(
                "sphere-result projection table is not allowlisted"
            )
        row = connection.execute(
            """
            SELECT sro.import_event_id, sro.snapshot_json, sro.total_gained, sro.stock,
        ...
        if row is None:
            raise SphereResultProjectionTargetError(
                f"projection target sphere_result_observations:{observation_id} is missing"
            )
        if int(row["import_event_id"]) != import_event_id:
            raise SphereResultProjectionTargetError(
                f"projection target sphere_result_observations:{observation_id} belongs to another import event"
            )
        import_event = connection.execute(
        ...
        try:
            ...
            raise SphereResultProjectionTargetError(
                "sphere-result projection slot is not valid JSON"
            ) from exc
        if (
            ...
            raise SphereResultProjectionTargetError(
                "sphere-result projection slot has an invalid shape"
            )
        if row["server"] != slot["server"] or row["account"] != slot["account"]:
            raise SphereResultProjectionTargetError(
                f"projection target sphere_result_observations:{observation_id} has mismatched sphere scope"
            )
        if row["snapshot_json"] != json.dumps(state.model_dump()):
            raise SphereResultProjectionTargetError(
                f"projection target sphere_result_observations:{observation_id} has mismatched snapshot"
            )
        if row["total_gained"] != state.total_gained or row["stock"] != state.stock:
            raise SphereResultProjectionTargetError(
                f"projection target sphere_result_observations:{observation_id} has mismatched sphere values"
            )

    @staticmethod
    def _sphere_result_slot(server: str, account: str) -> str:
        values = {
    ...
    def _positive_id(value: Any, field_name: str) -> int:
        if isinstance(value, bool) or not isinstance(value, int) or value <= 0:
            raise SphereResultProjectionIntegrityError(
                f"sphere-result import returned an invalid {field_name}"
            )
        return int(value)
