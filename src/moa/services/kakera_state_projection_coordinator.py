"""Transactional coordination for durable Discord Kakera-state projections."""
...
class KakeraStateProjectionCoordinatorError(RuntimeError):
    """Base error for a Kakera-state projection coordination failure."""


class KakeraStateProjectionStateError(KakeraStateProjectionCoordinatorError):
    """Raised when a source event or processing attempt is not usable."""


class KakeraStateProjectionIntegrityError(KakeraStateProjectionCoordinatorError):
    """Raised when durable Kakera-state projection state cannot be trusted."""


class KakeraStateProjectionTargetError(KakeraStateProjectionIntegrityError):
    """Raised when a Kakera-state projection target is missing or mismatched."""


class KakeraStateProjectionDatabasePathError(
    KakeraStateProjectionCoordinatorError, ValueError
):
    """Raised when the repositories do not point at one database."""


@dataclass(frozen=True, slots=True)
class KakeraStateProjectionResult:
    """The durable outcome of one coordinated Kakera-state projection."""
...
class KakeraStateProjectionCoordinator:
        ...
        try:
            ...
            if existing is not None:
                if str(existing["state"]) == "claimed":
                    raise KakeraStateProjectionIntegrityError(
                        f"Kakera projection for source event {source_event_id} is still claimed"
                    )
                raise KakeraStateProjectionIntegrityError(
                    f"source event {source_event_id} already has a Kakera projection"
                )
            ...
            imported = self._catalog._import_kakera_state_with_connection(
                connection,
                state=state,
                server=server,
                account=account,
            ...
            if success.attempt_status != "succeeded" or success.source_event_status != "succeeded":
                raise KakeraStateProjectionStateError(
            ...
            return KakeraStateProjectionResult(
                imported_count=1,
                import_event_id=import_event_id,
                kakera_state_observation_id=observation_id,
    ...
    def _coordinate_replay(
        ...
        if import_event is None or str(import_event["kind"]) != self._IMPORT_KIND:
            raise KakeraStateProjectionTargetError(
                f"legacy Kakera import event {import_event_id} for source event {event['id']} is missing or wrong"
            )
        ...
        if set(links) != {key}:
            raise KakeraStateProjectionIntegrityError(
                f"succeeded source event {event['id']} has an inconsistent Kakera projection link"
            )
        link = links[key]
        if str(link["state"]) != "completed":
            raise KakeraStateProjectionIntegrityError(
                f"Kakera projection for source event {event['id']} is not completed"
            )
        ...
        if projection_row_id is None:
            raise KakeraStateProjectionTargetError(
                f"Kakera projection for source event {event['id']} has no target"
            )
    ...
    def _complete_projection_link(
        ...
        *,
        ...
        if table not in self._TARGET_TABLES or isinstance(row_id, bool) or row_id <= 0:
            raise KakeraStateProjectionIntegrityError(
                "Kakera import returned an invalid projection target"
            )
        ...
        if updated.rowcount != 1:
            raise KakeraStateProjectionIntegrityError(
                "Kakera projection link could not be completed"
            )

    def _validate_kakera_target(
        self,
        connection: sqlite3.Connection,
        *,
        observation_id: int,
        ...
        if self._PROJECTION_TABLE not in self._TARGET_TABLES:
            raise KakeraStateProjectionIntegrityError(
                "Kakera projection table is not allowlisted"
            )
        ...
        if row is None:
            raise KakeraStateProjectionTargetError(
                f"projection target kakera_state_observations:{observation_id} is missing"
            )
        if int(row["import_event_id"]) != import_event_id:
            raise KakeraStateProjectionTargetError(
                f"projection target kakera_state_observations:{observation_id} belongs to another import event"
            )
        import_event = connection.execute(
        ...
        try:
            ...
            raise KakeraStateProjectionTargetError("Kakera projection slot is invalid") from error
        if (
            ...
            raise KakeraStateProjectionTargetError("Kakera projection slot is invalid")
    ...
    def _positive_id(value: Any, field_name: str) -> int:
        if isinstance(value, bool) or not isinstance(value, int) or value <= 0:
            raise KakeraStateProjectionIntegrityError(
                f"Kakera import returned an invalid {field_name}"
            )
        return int(value)
