...
class ProfileProjectionCoordinator:
    ...
    _PROJECTION_KIND = PROJECTION_EXPECTATION_POLICIES["profile"].possible_projection_kinds[0]
    ...
    def coordinate_profile(
        ...
        *,
        ...
        projection_slot = (
            resolve_expected_projections(
                build_projection_expectation_facts("profile", server=server, account=account)
            )
    ...
    def _complete_projection_link(
        ...
        *,
        ...
        if table not in self._TARGET_TABLES or isinstance(row_id, bool) or row_id <= 0:
            raise ProfileProjectionIntegrityError(
                "profile import returned an invalid projection target"
            )
