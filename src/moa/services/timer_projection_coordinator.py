"""Transactional coordination for durable Discord timer-state projections."""
...
class TimerProjectionCoordinatorError(RuntimeError):
    """Base error for a timer projection coordination failure."""


class TimerProjectionStateError(TimerProjectionCoordinatorError):
    """Raised when a source event or processing attempt is not usable."""


class TimerProjectionIntegrityError(TimerProjectionCoordinatorError):
    """Raised when durable timer projection state cannot be trusted."""


class TimerProjectionTargetError(TimerProjectionIntegrityError):
    """Raised when a timer projection target is missing or mismatched."""


class TimerProjectionDatabasePathError(TimerProjectionCoordinatorError, ValueError):
    """Raised when the repositories do not point at one database."""


@dataclass(frozen=True, slots=True)
class TimerProjectionResult:
    """The durable outcome of one coordinated timer-state projection."""
...
class TimerProjectionCoordinator:
        ...
        try:
            ...
            if existing is not None:
                if str(existing["state"]) == "claimed":
                    raise TimerProjectionIntegrityError(
                        f"timer projection for source event {source_event_id} is still claimed"
                    )
                raise TimerProjectionIntegrityError(
                    f"source event {source_event_id} already has a timer projection"
                )
            ...
            imported = self._catalog._import_timer_state_with_connection(
                connection,
                state=state,
                server=server,
                account=account,
            ...
            if success.attempt_status != "succeeded" or success.source_event_status != "succeeded":
                raise TimerProjectionStateError(
                    f"processing success was not recorded for source event {source_event_id}"
                )
            connection.commit()
            return TimerProjectionResult(
                imported_count=1,
                import_event_id=import_event_id,
                timer_state_observation_id=observation_id,
    ...
    def _coordinate_replay(
        ...
        if import_event is None or str(import_event["kind"]) != self._IMPORT_KIND:
            raise TimerProjectionTargetError(
                f"legacy timer import event {import_event_id} for source event {event['id']} is missing or wrong"
            )
        ...
        if set(links) != {key}:
            raise TimerProjectionIntegrityError(
                f"succeeded source event {event['id']} has an inconsistent timer projection link"
            )
        link = links[key]
        if str(link["state"]) != "completed":
            raise TimerProjectionIntegrityError(
                f"timer projection for source event {event['id']} is not completed"
            )
        ...
        if projection_row_id is None:
            raise TimerProjectionTargetError(
                f"timer projection for source event {event['id']} has no target"
            )
    ...
    def _complete_projection_link(
        ...
        *,
        ...
        if table not in self._TARGET_TABLES or isinstance(row_id, bool) or row_id <= 0:
            raise TimerProjectionIntegrityError("timer import returned an invalid projection target")
        ...
        if updated.rowcount != 1:
            raise TimerProjectionIntegrityError("timer projection link could not be completed")

    def _validate_timer_target(
        self,
        connection: sqlite3.Connection,
        *,
        observation_id: int,
        ...
        if self._PROJECTION_TABLE not in self._TARGET_TABLES:
            raise TimerProjectionIntegrityError("timer projection table is not allowlisted")
        row = connection.execute(
            """
            SELECT tso.import_event_id, ac.normalized_name AS account,
                   sc.normalized_name AS server
            FROM timer_state_observations AS tso
        ...
        if row is None:
            raise TimerProjectionTargetError(
                f"projection target timer_state_observations:{observation_id} is missing"
            )
        if int(row["import_event_id"]) != import_event_id:
            raise TimerProjectionTargetError(
                f"projection target timer_state_observations:{observation_id} belongs to another import event"
            )
        import_event = connection.execute(
    ...
    def _positive_id(value: Any, field_name: str) -> int:
        if isinstance(value, bool) or not isinstance(value, int) or value <= 0:
            raise TimerProjectionIntegrityError(f"timer import returned an invalid {field_name}")
        return int(value)
