"""Transactional coordination for durable Discord Tower-state projections."""
...
class TowerStateProjectionCoordinatorError(RuntimeError):
    """Base error for a Tower-state projection coordination failure."""


class TowerStateProjectionStateError(TowerStateProjectionCoordinatorError):
    """Raised when a source event or processing attempt is not usable."""


class TowerStateProjectionIntegrityError(TowerStateProjectionCoordinatorError):
    """Raised when durable Tower-state projection state cannot be trusted."""


class TowerStateProjectionTargetError(TowerStateProjectionIntegrityError):
    """Raised when a Tower-state projection target is missing or mismatched."""


class TowerStateProjectionDatabasePathError(
    TowerStateProjectionCoordinatorError, ValueError
):
    """Raised when the repositories do not point at one database."""


@dataclass(frozen=True, slots=True)
class TowerStateProjectionResult:
    """The durable outcome of one coordinated Tower-state projection."""
...
class TowerStateProjectionCoordinator:
        ...
        try:
            ...
            if existing is not None:
                if str(existing["state"]) == "claimed":
                    raise TowerStateProjectionIntegrityError(
                        f"Tower projection for source event {source_event_id} is still claimed"
                    )
                raise TowerStateProjectionIntegrityError(
                    f"source event {source_event_id} already has a Tower projection"
                )
            ...
            imported = self._catalog._import_tower_state_with_connection(
                connection,
                state=state,
                server=server,
                account=account,
            ...
            return TowerStateProjectionResult(
                imported_count=1,
                import_event_id=import_event_id,
                tower_state_observation_id=observation_id,
    ...
    def _coordinate_replay(
        ...
        if import_event is None or str(import_event["kind"]) != self._IMPORT_KIND:
            raise TowerStateProjectionTargetError(
                f"legacy Tower import event {import_event_id} for source event {event['id']} is missing or wrong"
            )
        ...
        if set(links) != {key}:
            raise TowerStateProjectionIntegrityError(
                f"succeeded source event {event['id']} has an inconsistent Tower projection link"
            )
        link = links[key]
        if str(link["state"]) != "completed":
            raise TowerStateProjectionIntegrityError(
                f"Tower projection for source event {event['id']} is not completed"
            )
        ...
        if projection_row_id is None:
            raise TowerStateProjectionTargetError(
                f"Tower projection for source event {event['id']} has no target"
            )
    ...
    def _complete_projection_link(
        ...
        *,
        ...
        if table not in self._TARGET_TABLES or isinstance(row_id, bool) or row_id <= 0:
            raise TowerStateProjectionIntegrityError(
                "Tower import returned an invalid projection target"
            )
        ...
        if updated.rowcount != 1:
            raise TowerStateProjectionIntegrityError(
                "Tower projection link could not be completed"
            )

    def _validate_tower_target(
        self,
        connection: sqlite3.Connection,
        *,
        observation_id: int,
        import_event_id: int,
        projection_slot: str,
        state: TowerStateSnapshot | None = None,
        observed_at: datetime | None = None,
    ) -> None:
        if self._PROJECTION_TABLE not in self._TARGET_TABLES:
            raise TowerStateProjectionIntegrityError(
                "Tower projection table is not allowlisted"
            )
        ...
        if row is None:
            raise TowerStateProjectionTargetError(
                f"projection target tower_state_observations:{observation_id} is missing"
            )
        if int(row["import_event_id"]) != import_event_id:
            raise TowerStateProjectionTargetError(
                f"projection target tower_state_observations:{observation_id} belongs to another import event"
            )
        import_event = connection.execute(
        ...
        try:
            ...
            raise TowerStateProjectionTargetError("Tower projection slot is invalid") from error
        if (
            ...
            raise TowerStateProjectionTargetError("Tower projection slot is invalid")
        if row["server"] != slot["server"] or row["account"] != slot["account"]:
            raise TowerStateProjectionTargetError(
                f"projection target tower_state_observations:{observation_id} has mismatched Tower scope"
            )
        if state is None:
            return
        ...
        if (
            ...
            raise TowerStateProjectionTargetError(
                f"projection target tower_state_observations:{observation_id} has mismatched Tower values"
            )
        try:
            built_perk_ids = json.loads(row["built_perk_ids_json"])
        except (TypeError, json.JSONDecodeError) as error:
            raise TowerStateProjectionTargetError(
                f"projection target tower_state_observations:{observation_id} has invalid built perks"
            ) from error
        if built_perk_ids != list(state.built_perk_ids):
            raise TowerStateProjectionTargetError(
                f"projection target tower_state_observations:{observation_id} has mismatched built perks"
            )
    ...
    def _positive_id(value: Any, field_name: str) -> int:
        if isinstance(value, bool) or not isinstance(value, int) or value <= 0:
            raise TowerStateProjectionIntegrityError(
                f"Tower import returned an invalid {field_name}"
            )
        return int(value)
